<!-- PM: "killParser4DCode"
Kills the process "parser4dCode" (just only when process still exist).
This method has no parameters ($0/$1/...) -->
#### `killParser4DCode`
---

## Description
Kills the process `parser4dCode` (just only when process still exist).<br>

This method has no parameters ($0/$1/...)<br>
Killing the process is only needed when you not need any longer to use
any of the component-methods in your session.
Start/Kill eats always 12ms, so start/restart the process only if needed.
Keep the process "parser4dCode" alive (not kill him) as long as possible,
so you can use always fast the other component-methods without delay of start/restart.
