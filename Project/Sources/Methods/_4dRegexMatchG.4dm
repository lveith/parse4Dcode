...
// PM: "_4dRegexMatchG" (new LV 21.05.20, 09:04:46)
...
// _4dRegexMatchG (srcTxt;regexPattern{;startPos{;withPosLengthResults}}) -> resultCollection with strings or with strings+postions+lengths
...
// UseExamples:
// - $resultCollection:=_4dRegexMatchG ("abc123xy98";"\\d")  // -> ["1","2","3","9","8"]
// - $resultCollection:=_4dRegexMatchG ("abc123xy98";"\\d+")  // -> ["123","98"]
// - $resultCollection:=_4dRegexMatchG ("abc123xy98";"\\d+";7)  // -> ["98"]
// - $resultCollection:=_4dRegexMatchG ("abc123xy98";"\\d+";1;True)  // -> [{strings:["123","98"]},{positions:[4,9]},{lengths:[3,2]}]
// - $resultCollection:=_4dRegexMatchG ("Dog called happyDog";"(HAPPY){0,1}(?i)DOG")  // -> ["Dog","Dog"]
// - $resultCollection:=_4dRegexMatchG ("Dog called happyDog";"(happy){0,1}(?i)DOG")  // -> ["Dog","happyDog"]
// - $resultCollection:=_4dRegexMatchG ("abc123 \r\n\t xy98";"\\p{C}")  // -> ["\r","\n","\t"] // find all cntrl-codes (not only CR, LF or TAB)
// - $resultCollection:=_4dRegexMatchG ("abc123 \r\n\t xy98";"[:cntrl:]")  // -> ["\r","\n","\t"] // find all cntrl-codes (not only CR, LF or TAB)
// - $resultCollection:=_4dRegexMatchG ("Dog called happyDog";"(?i)(HAPPY){0,1}DOG")  // -> ["Dog","happyDog"]
// - $resultCollection:=_4dRegexMatchG ("ABC123abc98";"abc")  // -> [abc]
// - $resultCollection:=_4dRegexMatchG ("ABCSS123abcß98";"(?i:abcss)")  // -> [ABCSS,abcß]
// - $resultCollection:=_4dRegexMatchG ("ABC123abc98";"(?-i:abc)")  // -> [abc]
// - $resultCollection:=_4dRegexMatchG ("aBxy123aBxY";"(?i)abxy")  // -> [aBxy,aBxY]
// - $resultCollection:=_4dRegexMatchG ("aBxy123aBxY";"(?i:ab)(?-i:xy)")  // -> [aBxy]
...
// - x - COMMENTS - If set, allow use of white space and #comments within patterns
// - s - DOTALL - If set, a "." in a pattern will match a line terminator in the input text. By default, it will not. Note that a carriage-return / line-feed pair in text behave as a single line terminator, 
// - m - MULTILINE - Control the behavior of "^" and "$" in a pattern. By default these will only match at the start and end, respectively, of the input text. If this flag is set, "^" and "$" will also match at the start and end of each line within the input text.
// - w - UWORD - Controls the behavior of \b in a pattern. If set, word boundaries are found according to the definitions of word found in Unicode UAX 29, Text Boundaries.
